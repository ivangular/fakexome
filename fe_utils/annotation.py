...
def get_seq_region_ids(cursor):
	...
	qry = "select name, seq_region_id from %s.seq_region where coord_system_id=4 " % ensembl_db_name['homo_sapiens']
...
def find_approved(cursor, stable_id):
	...
	qry += "from identifier_maps.hgnc where  ensembl_gene_id='%s'"%stable_id
...
ensembl_human = None
######
def annotate(cursor, line):
	# TODO: turn annotation into a class so we get rid of grossness like this:
	global seq_region_id
	global ensembl_human
	# coord_system with id 4 is chromosome for GRCh38
	if not seq_region_id: seq_region_id = get_seq_region_ids(cursor)
	if not ensembl_human:
		[all_species, ensembl_db_name] = get_species(cursor)
		ensembl_human  = ensembl_db_name['homo_sapiens']

	switch_to_db(cursor, ensembl_human)
